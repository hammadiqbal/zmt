...
function initializeMRSearch(selector, orgId, options) {
    options = options || {};
    var allowClear = options.allowClear !== undefined ? options.allowClear : true;
    var siteId = options.siteId || null;
    var getOrgId = options.getOrgId || null; // Function to dynamically get orgId
    var getSiteId = options.getSiteId || null; // Function to dynamically get siteId
    
    if ($(selector).hasClass('select2-hidden-accessible')) {
        $(selector).select2('destroy');
    }

    $(selector).html("<option value=''>Select MR #</option>").prop('disabled', false);
    ...
    $(selector).select2({
        ...
        ajax: {
            ...
            data: function (params) {
                // Dynamically get orgId or siteId if functions are provided
                ...
                // Add siteId or orgId (siteId takes precedence)
                if (currentSiteId) {
                    requestData.siteId = currentSiteId;
                } else if (currentOrgId) {
                    requestData.orgId = currentOrgId;
                }
            ...
            processResults: function (data, params) {
                ...
                var results = data.map(function(item) {
                    return {
                        id: item.mr_code,
                        text: item.mr_code + ' - ' + item.name + (item.cell_no ? ' - ' + item.cell_no : '')
                    };
                });

                var hasMore = results.length === 10;
        ...
        templateResult: function(data) {
            // Show loading message - return jQuery object, not HTML string
            if (data.loading) {
                var loadingText = data.text || 'Loading...';
                var $loadingDiv = $('<div>')
                    .css({
                        'padding': '8px',
                        'text-align': 'center',
            ...
            // Show formatted result - escape text to prevent HTML injection
            if (data.text) {
        ...
        templateSelection: function(data) {
            // Return plain text for selected value
            if (data.text) {
                return data.text;
            }
            return data.id || '';
    ...
    // Show initial loading state when dropdown opens
    // $(selector).on('select2:open', function() {
    //     // Select2 will automatically show "Searching..." when it fetches initial results
    // });
...
function openPatientInOutModal(mrNumber = '', billedamount = '', orgname = '', orgid = '', sitename = '', siteid = '', servicemode = '', servicemodeId = '', empname = '', empId = '', service = '', serviceId = '', billingcc = '', billingccId = '',patientstatusval = '',patientstatus = '',patientpriorityval = '', patientpriority = '',  locationname = '',  locationid = '',  schedulename = '',  scheduleid = '', scheduleStartTime = '', scheduleEndTime = '', pattern = '', remarks = '') {
    ...
    $('#enterMR').val('');
    initializeMRSearch('#enterMR', null);
    $('#patientArrivalDetails').hide();
    $("#pio_remarks").hide();
    ...
    if (mrNumber && billedamount && orgid && orgname && siteid && sitename && servicemodeId && service && serviceId && empname && empId && billingcc && billingccId && (!patientstatusval || !patientpriorityval || !locationid || !scheduleid)) {
        console.log('if');
        // Set MR number for Select2 with AJAX - need to create option first
        if (!$('#enterMR').find('option[value="' + mrNumber + '"]').length) {
            $('#enterMR').append('<option value="' + mrNumber + '">' + mrNumber + '</option>');
        }
...
$(document).ready(function() {
    ...
    $('#mr-selection-modal').on('show.bs.modal', function() {
        setTimeout(function() {
            ...
            initializeMRSearch('#sidebar-mr-select', null);
