        ...
        if (orgId) {
            ...
            fetchOrganizationSites(orgId, siteSelector, function(data) {
                if (data && data.length > 0) {
                    ...
                    else if(siteSelector == '#rot_source_site')
                    {
                        selectText = 'Select Source Site';
                    }
                     else if(siteSelector == '#rot_destination_site')
                    {
        ...
        .then(resp => {
            ...
            if (resp && Array.isArray(resp) && resp.length > 0) {
                ...
                if (selectedBatch) {
                    ...
                    // const $qty = currentRow.find('.id_qty');
                    if (Placeholder === 'newIssue' || Placeholder === 'respondMedication' || Placeholder === 'newOtherTransaction') {
                        if ($qty.length) {
...
function fetchActiveSL(siteId, element, inventoryStatus, successCallback, errorCallback ) {
...
function SiteChangeActivatedServiceLocation(siteSelector, locationSelector, formSelector, inventoryStatus) {
    // $(siteSelector).change(function() {
            ...
            $.ajax({
                ...
                success: function(resp) {
                    ...
                    if (resp && resp.length > 0) {
                        ...
                        Swal.fire({
                            text: 'Locations are not activated for the selected site',
                            icon: 'error',
                            confirmButtonText: 'OK'
                        }).then((result) => {
                            if (result.isConfirmed) {
...
$(document).on('click', '#urot_addmore', function() {
    var lastRow = $('.rot_details').last();
    var isFilled = true;
    lastRow.find('input, textarea, select').each(function() {
        if ($(this).val() === "" || $(this).val() === null) {
            $(this).addClass('requirefield');
    ...
    if(isFilled)
    {
        var index = ($('.uduplicate .rot_details').length);
        fetchInventoryGenerics('#u_rot_itemgeneric' + index, 'material', function(data) {
            var selectHtml = '<select class="form-control selecter p-0 u_rot_itemgeneric" name="u_rot_itemgeneric[]" id="u_rot_itemgeneric' + index + '" required style="color:#222d32">';
            if (data.length > 0) {
                $.each(data, function (key, value) {
                    selectHtml += `<option value="${value.id}">${value.name}</option>`;
                });
            ...
            var qtyField = '<div class="col-md-6">' +
                    ...
                    '<div class="form-group has-custom m-b-5">' +
                    '<label class="control-label">Demand Qty</label>' +
                    '<input type="number" class="form-control input-sm" placeholder="Enter Qty" required name="u_rot_qty[]">' +
                    '</div>' +
                    '</div>' +
                    '</div>' +
                    '</div>';
...
$(document).on('click', '#urmc_remove', function() {
    if ($('.uduplicate .rot_details').length > 1) {
        $('.uduplicate .rot_details').last().remove();
    }
    else {
        alert('You must have at least one more option.');
    }
...
$(document).on('click', '#urot_remove', function() {
    if ($('.uduplicate .rot_details').length > 1) {
        $('.uduplicate .rot_details').last().remove();
    }
    else {
