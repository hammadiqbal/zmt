...
$(function(){
  // create one tooltip div and append it to body
  var $tooltip = $('<div class="custom-tooltip"></div>').appendTo('body').hide();

  // when mouse enters a .balance-row, fill & show it
  $(document)
    .on('mouseenter', '.balance-row', function(){
      ...
      $tooltip
        .html('Org Balance: ' + org + '<br>Site Balance: ' + site)
        .fadeIn(150);
    })
...
function OrgChangeInventoryGeneric(orgSelector, genericSelector, formSelector) {
    // $(orgSelector).change(function() {
    ...
    // $(orgSelector).on('change.OrgChangeInventoryGeneric', function(){
        ...
        if (orgId) {
            ...
            const $form = $(formSelector);
            fetchOrganizationItemGeneric(orgId, genericSelector, function(data) {
                if (data && data.length > 0) {
                    $generic.empty()
...
function handleBatchNumberCheck(orgId, siteId, genericId, brandId, $brand, currentRow) {
    ...
    return $.getJSON('inventory/getbatchno', { orgId, siteId, genericId, brandId })
        .then(resp => {
            if (resp && resp.batch_no) {
                ...
                // $batch.val().prop('disabled', true).focus();
                ...
                if (!$brand.data('warning-shown')) {
                    Swal.fire('No batch# found', 'No inventory for that combination.', 'warning');
                    $brand.data('warning-shown', true);
                    
                    // Reset the warning flag after 1 second
                    setTimeout(() => {
                        $brand.removeData('warning-shown');
                    }, 1000);
                }
                ...
                currentRow.find('.id_batch').val('').prop('disabled', true)
                currentRow.find('.id_expiry').val('').prop('disabled', true);
    ...
    $(siteSelector).off('change.SiteChangeEmployees').on('change.SiteChangeEmployees', function(){
        ...
        if (siteId) {
            ...
            const handleEmployeeData = function(data, emptyMessage, modalSelector, placeholder) {
                ...
                if (data && data.length > 0) {
                    alertShown = false; 
                    $emp.append(data.map(({ id, name }) => `<option value="${id}">${name}</option>`).join(''))
                        .prop('disabled', false);
                    // $emp.trigger('change');
                } else {
...
function SiteChangeMaterialManagementTransactionTypes(siteSelector,orgSelector, TransactionTypeSelector, formSelector,condition) {
    // $(siteSelector).change(function() {
    $(siteSelector).off('change.SiteChangeMaterialManagementTransactionTypes').on('change.SiteChangeMaterialManagementTransactionTypes', function(){
        ...
        if (orgId) {
            const $TransactionType = $(TransactionTypeSelector);
            const $form = $(formSelector);
            fetchMaterialManagementTransactionTypes(orgId, TransactionTypeSelector,condition, function(data) {
                if (data && data.length > 0) {
                    $TransactionType.empty()
                         .append('<option selected disabled value="">Select Transaction Type</option>')
                         .append(data.map(({id, name}) => `<option value="${id}">${name}</option>`).join(''))
            ...
            fetchServiceScheduling(locationId, siteId, ScheduleSelector, function(data) {
                if (data && data.length > 0) {
                    $schedule.empty()
                        ...
                        .append(data.map(({ id, name, start_timestamp, end_timestamp }) => {
                            ...
                            return `<option value="${id}">${name} (StartTime: ${formattedStartDate} - EndTime: ${formattedEndDate})</option>`;
                            // return `<option value="${id}">${name} (StartTime: ${formattedStartDate} - EndTime: ${formattedEndDate}</option>)`;
                        }).join(''))
                        .prop('disabled', false)
                        .find('option:contains("Loading...")').remove();
        ...
        if (siteId) {
            ...
            fetchPatientMR(siteId, MRSelector, condition, function(data) {
                if (data && data.length > 0) {
                    ...
                    if ($('.mr_optional').is(':hidden')) { // Check if MR# is required
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                $MR.empty()
                                    .append('<option selected disabled value="">Select Patient MR#</option>')
                                    .prop('disabled', true);
                                if ($('#mc_patient').prop('required')) {
                                    $('#mc_transactiontype').val('').trigger('change');
                                }
                            }
                        });
                    }
        ...
        success: function(response) {
            ...
            if (response && response.CostCenters.length > 0 && response.Services.length > 0) {
                ...
                response.Services.forEach(service => {
                    ...
                    const billingCostCenterList = billingCostCenters.join('<br>');
                    const performingCostCenterList = performingCostCenters.join('<br>');
                    const serviceModesList = serviceModes.join('<br>');
                    rows += `
...
function OrganizationChangServiceLocation(orgSelector, locationSelector, formSelector) {
    // $(orgSelector).change(function() {
    $(orgSelector).off('change.OrganizationChangServiceLocation').on('change.OrganizationChangServiceLocation', function(){
        var orgId = $(orgSelector).val();
        if (orgId) {
            $('.emp_location').hide(); 
...
function fetchMaterialManagementTransactionTypes(orgId, element, condition, successCallback, errorCallback) {
    let requestData = {
        orgId: orgId,
        condition: condition 
    };
...
function fetchOrganizationItemGeneric(orgId, element, successCallback, errorCallback) {
    let requestData = {
        orgId: orgId,
    };
...
function fetchActiveSL(siteId, element, successCallback, errorCallback) {
    let requestData = {
        siteId: siteId  
    };
    $.ajax({
...
function SiteChangeActivatedServiceLocation(siteSelector, locationSelector, formSelector) {
    // $(siteSelector).change(function() {
            ...
            $.ajax({
                ...
                success: function(resp) {
                    ...
                    if (resp && resp.length > 0) {
                        ...
                        Swal.fire({
                            text: 'Service locations are not activated for the selected site',
                            icon: 'error',
                            confirmButtonText: 'OK'
                        }).then((result) => {
                            if (result.isConfirmed) {
        ...
        if (typeId) {
            ...
            fetchSelectedInventoryGeneric(typeId, genericSelector, function(data) {
                if (data && data.length > 0) {
                    ...
                    Swal.fire({
                        text: 'Item Generics are not available for selected Transaction Type',
                        icon: 'error',
                        confirmButtonText: 'OK'
                    }).then((result) => {
                        if (result.isConfirmed) {
...
$(document).on('click', '#umc_addmore', function() {
    ...
    if(isFilled)
    {
        ...
        fetchInventoryGenerics('#u_mc_itemgeneric' + index, 'material', function(data) {
            var selectHtml = '<select class="form-control selecter p-0 u_rmc_inv_generic" name="u_mc_itemgeneric[]" id="u_mc_itemgeneric' + index + '" required style="color:#222d32">';
            if (data.length > 0) {
                $.each(data, function (key, value) {
                    selectHtml += `<option value="${value.id}">${value.name}</option>`;
                });
...
$(document).on('click', '#urmc_remove', function() {
    if ($('.uduplicate .rmc_details').length > 1) {
        $('.uduplicate .rmc_details').last().remove();
    }
    else {
        alert('You must have at least one more option.');
    }
...
$(document).on('click', '.ulocation_ela_value', function () {
    ...
    var locationValues = uactiveRowServiceLocation.find('input[name="ulocation_ela_value"]').val();
    var OrgId = uactiveRowServiceLocation.find('input[name="ulocationOrg"]').val();
            ...
            data.forEach(item => {
                var embedData = `
                    <tr style="font-size:14px;cursor:pointer;">
                        <td>
                            <div class="custom-control custom-checkbox p-1">
                                <input type="checkbox" name="uselectedServiceLocation[]" data-id="${item.id}" data-name="${item.name}" class="custom-control-input" id="uela_${item.id}">
                                <label class="custom-control-label" for="uela_${item.id}"></label>
                            </div>
                        </td>
                        <td>${item.name}</td>
                    </tr>`;
...
async function LoadEmpAllocatedLocation(EmployeeId) {
    try {
        ...
        for (let siteIndex = 0; siteIndex < locationSites.length; siteIndex++) {
            ...
            console.log(`${locationSites[siteIndex].siteId}`);
            console.log(`${locationSites[siteIndex].locationSiteName}`);
                ...
                <div class="sl-item">
                    ...
                    <div class="row loademplocation">
                        ...
                        <div class="link col-lg-6">
                            ...
                            <input type="text" class="form-control cursor-pointer mb-2 ulocation_ela_value" name="ulocation_ela_value" value="${locationSites[siteIndex].serviceLocations}" readonly>
        ...
        <div class="sl-item">
            <div class="row">
                ...
                <div class="link col-lg-6">
                     <input type="hidden" value="${orgId}" name="ulocationOrg">
                    <label class="control-label font-weight-bold">Service Locations</label>
                    <input type="text" class="form-control cursor-pointer mb-2 ulocation_ela_value" placeholder="Select Service Locations" name="ulocation_ela_value" readonly>
                    <input type="hidden" name="ulocation_ela[]" >
                </div>
        </div>
        <hr>`;
...
async function LoadEmployeeCostCenter(EmployeeId) {
    try {
        ...
        const heacCountSiteId = response.heacCountSiteId;
        ...
        // const heacCountSiteIds = response.heacCountSiteId.split(",");
        ...
        for (let CCIndex = 0; CCIndex < CostCenterIds.length; CCIndex++) {
            ...
            let CCData = await fetchActivatedCostCentersPromise(heacCountSiteId, '.u_empcc', CostCenterIds[CCIndex]);
            ...
            entryHtml += `
                ...
                <div class="row loadempcc">
                    ...
                    <div class="link col-lg-3">
                        ...
                        <div class="form-group has-custom m-b-5">
                            <input type="number" class="form-control input-sm mb-2" value="${CostCenterPercentages[CCIndex]}" name="u_ecc_percent[]" max="100" min="0" id="input0112">
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="sl-item">
            <div class="row">
                ...
                <div class="link col-lg-3">
                    <label class="control-label font-weight-bold">Cost Center Percentage</label>
                    <div class="form-group has-custom m-b-5">
                        <input type="number" class="form-control input-sm mb-2" placeholder="Enter Percentage" name="u_ecc_percent[]" max="100" min="0" id="input0112">
                    </div>
                </div>
            </div>
        </div>
...
function updateHiddenAllServiceLocation() {
    ...
    $('input[name="inventory_location_value"]').val(selectedNames.join(','));
    $('input[name="inventory_location[]"]').val(selectedIds.join(','));
}
...
function updateHiddenUpdatedServiceLocation() {
    var selectedIds = [];
    var selectedNames = [];
    ...
    $('input[name="u_inventory_location[]"]').val(selectedIds.join(','));
    $('input[name="u_inventorylocation_value"]').val(selectedNames.join(','));
}
...
function handleRequisitionEPISubmission(formId,modalId) {
    ...
    $(data).each(function(i, field) {
        ...
        {
            var FieldName = field.name;
            var FieldClass = '.' + FieldName + "_error";
...
function openPatientInOutModal(mrNumber = '', billedamount = '', orgname = '', orgid = '', sitename = '', siteid = '', servicemode = '', servicemodeId = '', empname = '', empId = '', service = '', serviceId = '', billingcc = '', billingccId = '',patientstatusval = '',patientstatus = '',patientpriorityval = '', patientpriority = '',  locationname = '',  locationid = '',  schedulename = '',  scheduleid = '') {
    ...
    $('#patientArrivalDetails').hide();
    $("#pio_remarks").hide();
    // $('#enterMR').val(''); // Clear MR input
    // $('#enterMR').val('').prop('disabled', false);
