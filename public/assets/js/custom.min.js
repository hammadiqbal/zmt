  ...
  $(document)
    .on('mouseenter', '.balance-row', function(){
      var $row = $(this),
        ...
        locs = $row.data('loc-balance'); // array
        var locText = '';
      ...
      $tooltip
        .html('Org Balance: ' + org + '<br>Site Balance: ' + site + '<br>' +locText)
        .fadeIn(150);
    })
        ...
        .then(resp => {
            ...
            if (resp && Array.isArray(resp) && resp.length > 0) {
                ...
                if (selectedBatch) {
                    ...
                    currentRow.find('.id_expiry_error').text('');
                    console.log(selectedBatch.availableQty);
                    ...
                    // else if (Placeholder === 'respondMaterial') {
                    //     if ($qty.length) {
                    //         $qty.attr('max', selectedBatch.site_balance);
                    //         $qty.attr('placeholder', `Max: ${selectedBatch.site_balance} (Available: ${selectedBatch.site_balance}, Used: ${selectedBatch.usedQty})`);
                    //     }
                    // }
                } else {
                    Swal.fire({
                        title: 'No Inventory Left',
                        text: 'All available batches are exhausted.',
                        icon: 'warning',
                        confirmButtonText: 'OK',
                        allowOutsideClick: false
                    }).then(() => {
...
function BrandChangeBatchAndExpiry(org, site, generic, brand, batch, expiry, respond, form) {
    const $org = $(org);
    const $site = $(site);
    const $gen = $(generic);
    const $brand = $(brand);
    ...
    //     .on('change.BrandChangeBatch', function(e) {
        $brand.on('change.BrandChangeBatch', function(e) {
            ...
            handleBatchNumberCheck(orgId, siteId, genericId, brandId, $brand, currentRow, respond); 
