$(document).ready(function() {
    //Open Confirm Sample
    $(document).on('click', '.confirm-sample', function() {
    ...
    //Add Sample Collection Conformation
    $('#confirm_sample').submit(function(e) {
        e.preventDefault();
        // var data = SerializeForm(this);
        var data = new FormData(this);
        $.ajax({
            url: "/medicalrecord/sampleconfirmationdate",
            method: "POST",
            headers: {
                    ...
                    if (!successMessageShown) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                $('#view-investigationTracking').DataTable().ajax.reload();
                                $('#confirm_sample')[0].reset();
                                $('#confirm-sample').modal('hide');
    ...
    $('#upload_report').submit(function(e) {
        e.preventDefault();
        // var data = SerializeForm(this);
        var data = new FormData(this);
        $.ajax({
            url: "/medicalrecord/uploadreport",
            method: "POST",
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
    ...
    // View Investigation Tracking
    var pathArray = window.location.pathname.split('/');
    var mrPath = pathArray.length - 1;
    var mrPath = pathArray[mrPath];
    var viewReqMC =  $('#view-investigationTracking').DataTable({
        processing: true,
        serverSide: true,
        ajax: '/medicalrecord/viewinvestigationTracking/' + mrPath,
        order: [[1, 'desc']],
        columns: [
    ...
    // Download Report
    $(document).on('click', '.downloaditReport', function () {
        var attachmentPaths = $(this).data('path'); 
        var id = $(this).data('id');
        ...
        if (attachmentPaths) {
            ...
            files.forEach(function (file) {
                ...
                tempLink.href = '/assets/investigationtracking/' + id + '_' + file.trim();
                tempLink.download = file.trim(); 
