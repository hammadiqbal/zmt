$(document).ready(function() {
    ...
    // $(document).on('click', '.add-consumption', function() {
    //     $('#consumption_dl,#consumption_sl,.serviceDetails,.brand_details').hide();
    //     $('input[name="consumption_reference_document"]').val('').prop('disabled', false);
    //     $('.req_only').show();
    //     $('#consumption_mr').closest('.col-md-6').show();
    //     $('#transaction-info-row').empty();
    //     $('#mrService,.mr-dependent').hide();
    //     $('#consumption_mr').val('');
    //     $('.consumption_brand').off('change.BrandChangeBatch');
    //     $('.consumption_qty').attr('max', 0);
    //     $('.consumption_qty').attr('placeholder', 'Transaction Qty..');
    ...
    //                 id: 'source_type',
    //                 value: 'material'
    //             }).appendTo('#add_consumption');
    //         }

    //     $(".consumption_frequency, .consumption_route").each(function() {
    //         $(this).val($(this).find("option:first").val()).change();
    //     });
    //     var orgId = $('#consumption_org').val();
    //     $('#consumption_transactiontype').html("<option selected disabled value=''>Select Transaction Type</option>").prop('disabled',true);
    //     if(orgId)
    //     {
    //         $('#consumption_site').html("<option selected disabled value=''>Select Site</option>").prop('disabled',false);
    //         fetchOrganizationSites(orgId, '#consumption_site', function(data) {
    //             $.each(data, function(key, value) {
    //                 $('#consumption_site').append('<option value="' + value.id + '">' + value.name + '</option>');
    //             });
    //         });
    //         fetchOrgPatient(orgId, '#consumption_mr', function(data) {
    //             $('#consumption_mr').html("<option selected disabled value=''>Select MR #</option>").prop('disabled', false);
    //             $.each(data, function(key, value) {
    //                 $('#consumption_mr').append('<option value="' + value.mr_code + '">' + value.mr_code + ' - ' + value.name +'</option>');
    //             });
    //         });
    //         $('.consumption_generic').html("<option selected disabled value=''>Select Item Generic</option>").prop('disabled', false);
    //         fetchOrganizationItemGeneric(orgId, '.consumption_generic', function(data) {
    //             $.each(data, function(key, value) {
    //                 $('.consumption_generic').append('<option value="' + value.id + '">' + value.name + '</option>');
    //             });
    //         });
    //     }
    //     else{
    //         $('#consumption_org').html("<option selected disabled value=''>Select Organization</option>").prop('disabled',false);
    //         fetchOrganizations('null', '','#consumption_org', function(data) {
    //             $('#consumption_org').find('option:contains("Loading...")').remove();
    //             $.each(data, function(key, value) {
    //                 $('#consumption_org').append('<option value="' + value.id + '">' + value.organization + '</option>');
    //             });
    //         });
            
    //         $('#consumption_site').html("<option selected disabled value=''>Select Site</option>").prop('disabled',true);
    //         OrgChangeSites('#consumption_org', '#consumption_site', '#add_consumption');
            
    //         $('#consumption_mr').html("<option selected disabled value=''>Select MR #</option>").prop('disabled', true);
    //         SiteChangeMRCode('#consumption_site', '#consumption_mr', null);
            
    //         $('.consumption_generic').html("<option selected disabled value=''>Select Item Generic</option>").prop('disabled', true);
    //         OrgChangeInventoryGeneric('#consumption_org', '.consumption_generic', '#add_consumption');
    //     }
    //     SiteChangeMaterialManagementTransactionTypes('#consumption_site','#consumption_org', '#consumption_transactiontype', '#add_consumption','issue_dispense','n');
    ...
    //     if ($('#consumption_mr').val()) {
    //         toggleDuplicateFieldsBasedOnMR($('#consumption_mr').val());
    //     }
    ...
    //     $('.consumption_brand').html("<option selected disabled value=''>Select Item Brand</option>").prop('disabled', true);

    //     $(document).off('change.newIssueBrand').on('change.newIssueBrand', '.consumption_brand', function(e) {
    //         e.stopPropagation();
            
    //         const currentRow = $(this).closest('.duplicate');
    //         const orgId = $('#consumption_org').val();
    //         const siteId = $('#consumption_site').val();
    //         const genericId = currentRow.find('.consumption_generic').val();
    ...
    //         handleBatchNumberCheck(orgId, siteId, genericId, brandId, currentRow, 'newIssue','#add-consumption', {batchSelector: '.consumption_batch',brandSelector: '.consumption_brand', qtySelector: '.consumption_qty', expirySelector: '.consumption_expiry'});
    //     });

    //     $('#add-consumption').modal('show');
    // });
  
    // $(document).off('change', '.consumption_generic').on('change', '.consumption_generic', function() {
    //     var genericId = $(this).val();
    //     var currentRow = $(this).closest('.duplicate'); 
    //     var currentRowBrandSelect = currentRow.find('.consumption_brand'); 
    ...
    //                         var $qty = currentRowBrandSelect.closest('.duplicate').find('.consumption_qty');
    //                         if ($qty.length) {
    //                             $qty.removeAttr('max');
    ...
    // View Issue Dispense
    var viewIssueDispense =  $('#view-consumption').DataTable({
        processing: true,
        serverSide: true,
